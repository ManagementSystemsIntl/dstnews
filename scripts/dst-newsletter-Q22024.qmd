...
# grViz("
      ...
      # 
      # node[shape=box, color='darkseagreen1', style='filled', fontsize=8]
      # 
      # edge[penwidth=.6]
      # #{B C} -> A
      # B -> A
...
set.seed(342)
n <- 1000
talent <- rbinom(n, 1, .5) # a thousand flips of a fair coin
attractiveness <- rbinom(n, 1, .5) # a thousand flips of a fair coin
...
```{r}
actor <- rbinom(n, 1, ifelse(talent + attractiveness > 0, .9, .1)) 
...
Note the different direction of the arrows between omitted variable bias in the first graph, and collider bias in the second graph.

The critical learning points are to (a) learn to look out for collider variables in your work, and (b) DO NOT include collider variables in your analysis!
